$(function(){
	$(".no-down").click(function(){
		$(".download").hide();
	})//swiperContainer必选，HTML元素或者string类型，Swiper容器的css选择器
...
// 　　　　effect:"flip"//--可以实现3D效果的轮播
	...
	// 			str+='<li><a href="javascript:void(0)">'+res.popular_diseases[i].name+'</a></li>'
 //    		}
	// 	},
	// 	error: function(res){
	// 		console.log(res);
	...
	//图片轮播
	// var pic = "";
	// for(var p=0;p<res.banner_list.length;p++){
	// 	pic+='<a href="'+res.banner_list[p].url+'" class="swiper-slide"><img src="'+res.banner_list[p].image+'" alt=""></a>';
	// }
	// $(".swiper-wrapper").html(pic);
	//疾病科普
	var str = "";
	for(var i=0;i<res.popular_diseases.length;i++){
		str+='<li><a href="./sick.html?id='+res.popular_diseases[i].id+'">'+res.popular_diseases[i].name+'</a></li>';
	}
	$(".sick-ul").html(str);
	//健康咨询
	var strhea = "";
	for(var j=0;j<res.health_news.length;j++){
		strhea+='<a href="javascript:void(0);" class="health-list"><img src="'+res.health_news[j].image+'" alt=""><div class="health-right"><h6 class="title health-title">'+res.health_news[j].title+'</h6><div class="health-foot"><span>'+res.health_news[j].channel_name+'</span><span>'+res.health_news[j].date_str+'</span></div></div></a>';
	}
	$(".health").html(strhea);
	// 经典问答
	var strhot = "";
	for(var k=0;k<res.hot_problems.length;k++){
		strhot+='<a href="javascript:void(0);" class="first ask-list"><h6 class="title"><span class="title-logo">问</span>'+res.hot_problems[k].title+'</h6><p class="user-title">'+res.hot_problems[k].ask+'</p></a>';
	}
	$(".ask").html(strhot);
})